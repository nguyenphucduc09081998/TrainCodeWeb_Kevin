 ...
 */
 ...
 function update_user(){
	 ...
	 <input type="hidden" name="action" value="edit_profile">
			  <input type="submit" value="update_usser">
			   <br><br><br><br><br><br>

<?php
			  }
 add_shortcode('abc', 'update_user');
...
<form action=<?php echo esc_url( admin_url('admin-post.php') ); ?> method="post" enctype="multipart/form-data">			
		...
		wp_redirect('http://wordpress.test/student-management/');
	 }
	 else{
		 $offset= $_GET['offset'];
...
<input type="hidden" name="action" value="export_csv">
	...
	while ($query->have_posts()) {
		...
		?>
		 ...
		 <button style="padding:2px;" onclick="window.location.href='<?php echo home_url( $wp->request ).'/student-management?'.'?p=edit&id='.$post_id?>'">Edit</button>
		 <button style="padding:2px;" onclick="window.location.href='<?php echo home_url( $wp->request ).'/student-management?'.'?p=delete&id='.$post_id?>'">Del</button>
		 </td>
		<?php
