...
'<label>Student profile</label>:</br> '.'<image width="480" height="480" src="'. get_post_meta($_POST['id'], 'student_profile', TRUE).'"></image>';
...
function my_plugin_menu() {
	add_options_page( 'Admin Menu', 'Admin Menu', 'manage_options', 'my-unique-identifier', 'my_plugin_options' );
}
...
function my_plugin_options() {
	...
	Pagination number:<label style="color:red"><?php echo get_option('offset')?></label> <input type="input" name="pagination" > 
...
function check_values( $post_id, $post, $update ){
	...
	//update_user_meta($post_id, $post, $update);
}
function check_login(){
    $user_id = get_current_user_id();
    update_user_meta($user_id, 'time-login', time());
}
function set_option(){
	update_option( 'offset', $_POST['pagination']);
	wp_redirect('http://wordpresstr.test/wp-admin/options-general.php?page=my-unique-identifier');
}
...
add_action( 'admin_post_admin_menu_custom','set_option' );
add_action( 'admin_post_nopriv_admin_menu_custom','set_option' );
